...
class HistoryRepository {
  ...
  Future<void> savePosition(String urlHash, int positionMs, int durationMs) async {
    final existing = await _db.query('playback_history').whereEqual('url_hash', urlHash).first();
    if (existing != null) {
      await _db
  ...
  Future<PlaybackHistory?> getLastPosition(String urlHash) async {
    final result = await _db.query('playback_history').whereEqual('url_hash', urlHash).first();
  ...
  Future<List<PlaybackHistory>> getRecentHistory({int limit = 50}) async {
    final results = await _db.query('playback_history').orderByDesc('played_at').limit(limit);
  ...
  Future<void> clearAll() async {
    await _db.clear('playback_history');
  }
}
